...
from cognitive_core.models import NeuroStateNormalized

logger = logging.getLogger(__name__)

_NEUROSTATE_ROOT = os.path.abspath(
    os.path.join(os.path.dirname(__file__), "..", "..", "neurostate-engine")
)

# neurostate-engine を sys.path の先頭に挿入してから import する。
# cognitive-layer は "core" という名前のパッケージを持たないため競合しない。
if _NEUROSTATE_ROOT not in sys.path:
    sys.path.insert(0, _NEUROSTATE_ROOT)
